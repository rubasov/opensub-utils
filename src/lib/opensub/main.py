...
1. calculate hash of movie: `file_hash()`

2. search for subtitles by: `UserAgent().search()`
...
3. download and access subtitles
    ...
    * get them as file-like objects: `SubtitleArchive().yield_open()`
    * extract them next to the movie files: `SubtitleArchive().extract()`
...
* file hash
  ...
  The hash of a file as defined by (originally, I think) Movie Player
...
* template
  ...
  Template to use during subtitle extraction. Think of renaming files while
...
* Using simplexml interface to opensubtitles.org instead of xml-rpc api.
  ...
  By using xml-rpc we could:
      ...
      * make mass searches perform better
...
* Identifying multi-cd movies by the hash of the first file and cd count.
  ...
  The basic choices were:
      ...
      2. Handle standalone video/subtitle files as a unit,
         i.e. identify each file by its own file hash.
  ...
  coming from different sources, for example from different translators.
